...
The three CSVs imported above are appended together. The PIBI was developed for the taxa found above the pycnocline. Therefore, rows where the layer column does not specifies above the pycnocline ("ap") or water column ("wc") are removed. Furthermore, [Lacouture et al. (2006 pg. 599)](https://www.potomacriver.org/focus-areas/aquatic-life/phytoplankton-index-of-biotic-integrity-for-chesapeake-bay/) developed indices for data collected in the spring (March-May) and summer (July-September).
```{r}
bay.df <- bind_rows(lrr.df, odu.phyto, odu.pico) %>% 
...
bay.df <- bay.df %>% 
  ...
  filter(grepl("ph", method),
         !latinname %in% c("micro-phytoflagellates", "microflagellates",
                           "green_cells",
